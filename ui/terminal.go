...
// Updates the terminal pane content with the tmux terminal session content
func (t *TerminalPane) UpdateContent(instance *session.Instance) error {
	switch {
	case instance == nil:
		t.setFallbackState("No agents running yet. Spin up a new instance with 'n' to get started!")
		return nil
	...
	// Get terminal content from the instance's working directory
	content, err := instance.TerminalPreview()
	if err != nil {
		return err
	}

	if len(content) == 0 && !instance.Started() {
		t.setFallbackState("Please enter a name for the instance.")
		return nil
	}
...
// Returns the terminal pane content as a string.
